...
If you need a helper function to  read in your VCF or LD matrix file we recommend crosshap (Marsh et al., 2023), which has inbuilt functions for this purpose.

In this tutorial we will use the datasets provided with the tool.
...
**LD** The LD matrix is a table with the dimensions n × n, where n represents the number of individuals in the population. The LD matrix can be made using any pairwise LD metric such as D (Robbins, 1918), D′ (Lewontin, 1964), or r2 (Hill & Roberston, 1968).Your file should look similar to the following:
...
Format 2 is compatible with some GWAS tools. The numbers represent the number of copies present for each haplotype variants. The haplotype can either be absent from that individual (0), present in one strand (1) or present in both strands (2).
